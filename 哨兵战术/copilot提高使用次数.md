...
把文末的markdown信息保存成一个新的Agent描述文档即可使用

![](https://i2.hdslb.com/bfs/new_dyn/2f77ffaf8113f2e882ad1e75a1c7ffc94034116.png@736w_568h.webp)

## 建议

把vs code里chat最大请求数改成： 200-opus（或相同等级，用乘数确认，例如3x 1x 0.33x） 100-sonnet（1x等级）

50-默认值（1x等级以下） 修改成其他自定义值的风险要自己承担，**过高的请求数、请求频次和单轮请求量**可能会触发限制导致无法使用高级模型，这里的数值仅为经验之谈，如果频繁遇到限流，建议减少一点 这个数值并非越大越好，过多信息挤在一个对话里会显著降低产出质量并引发幻觉问题

![](https://i2.hdslb.com/bfs/new_dyn/db7779a31b651223fe6d72e1df12682d4034116.png@1192w.webp)

## 注意事项

1.  只要有可能，就去用最顶级模型，除非由于拥挤或者限速等原因。本文成型时，最高级别模型是Claude Opus 4.6，可以通过请求乘数来确认等级（如3x），否则
    
2.  模型建议至少是sonnet 4.5，低级模型可能不会调用询问工具；vs code版本太低没有询问工具也不行
    
3.  不建议追问过快，如前文所述，单位时间最大请求频率一直以来都有限制，建议每2分钟至多提问一次，并且你可以追问时一次性描述多个问题，一起解决，避免请求过多
    
4.  **建议在完全结束对话后新开一个对话使用Endless模式，不要继续对话**，否则Agent可能不会遵循之前的要求；如果开始压缩对话，就更不能在当前对话结束后继续追问了，因为有很高概率遗忘你的要求
    
5.  **OpenCode和OMO体系不适用此描述文件，因为它们的追问工具可能算作一个新的请求，建议使用**：https://github.com/jiwangyihao/opencode-copilot-account-switcher
    
6.  对于Copilot CLI，需要稍微变通一下，把本文喂给Agent，让他改用他可用的追问方式，而不是VS Code的提问工具
    
7.  **不能在已经开启的对话里使用Endless模式**，请使用一个新开的对话进行
    

## 效果

每次完成一次作业，chat都会出现一个对话框询问你下一步做什么，而不是直接停止，因此可以开始下一个任务而不需要花费一个新的可用次数 当然了：无法超过系统的最大限制（就是上文所说的请求数，虽然是你配置的，但我建议你最好保持建议值，超过质量会严重下降），建议达到80%最大请求数就可以考虑下一轮了 由于copilot的请求算法并不明确，且AI可能自己的计算过程也会干扰计数，因此可能会出现超出所描述的最大请求数还在运行的情况，建议酌情使用，这种状态下随时可能会被系统打断，不一定能完整完成你的需求

![](https://i2.hdslb.com/bfs/new_dyn/0ea66b606fe3587b28f69399a76119224034116.png@1192w.webp)
