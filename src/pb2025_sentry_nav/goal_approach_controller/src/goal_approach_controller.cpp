...
public:
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) override
  {
    ...
    RCLCPP_INFO(
      logger_,
      "GoalApproachController wrapping %s, approach_distance=%.2f, approach_velocity=%.2f, direct_approach_distance=%.2f, direct_approach_kp=%.2f",
    ...
    nav2_core::GoalChecker * goal_checker) override
  {
    ...
    const double dx = goal_.pose.position.x - pose.pose.position.x;
    const double dy = goal_.pose.position.y - pose.pose.position.y;
    const double distance = std::hypot(dx, dy);

    if (distance < direct_approach_distance_) {
      const double target_speed = std::min(approach_velocity_, distance * direct_approach_kp_);
      if (distance > 0.01) {
        cmd.twist.linear.x = target_speed * (dx / distance);
        cmd.twist.linear.y = target_speed * (dy / distance);
      } else {
