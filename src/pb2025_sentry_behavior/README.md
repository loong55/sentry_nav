...
┌─────────────────────────────────────────────────────────────────┐
│                        外部传感器/系统                             │
│  裁判系统 │ 视觉检测 │ 导航系统 │ 底盘控制 │ 云台控制              │
└────┬────────┬────────┬────────┬────────┬─────────────────────────┘
     │        │        │        │        │
     ▼        ▼        ▼        ▼        ▼
┌─────────────────────────────────────────────────────────────────┐
│              ROS2 话题 (Topics)                                   │
│  /referee/*  /detector/*  /tracker/*  /cmd_vel  /gimbal_cmd     │
└────────────────────┬────────────────────────────────────────────┘
                     │
                     ▼
     ┌───────────────────────────────────────┐
     │   SentryBehaviorServer (服务端)        │
     │   - 订阅所有传感器话题                   │
     │   - 数据存入全局黑板                     │
     │   - 加载并执行行为树                     │
     │   - 提供Action Server接口               │
...
└────────────────────┬───────────────────────────────────────────┘
                     ...
                     ▼
┌────────────────────────────────────────────────────────────────┐
│              服务端订阅器（subscriptions_）                      │
│  [GameStatus] [RobotStatus] [Armors] [Target] [Costmap] ...    │
└────────────────────┬───────────────────────────────────────────┘
                     │ (回调函数)
                     ▼
┌────────────────────────────────────────────────────────────────┐
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ "@referee_gameStatus" → GameStatus消息                    │ │
│  │ "@referee_robotStatus" → RobotStatus消息                  │ │
│  │ "@detector_armors" → Armors消息                           │ │
│  │ "@nav_globalCostmap" → OccupancyGrid消息                  │ │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────┬───────────────────────────────────────────┘
                     │ (黑板读取)
                     ▼
...
│                   行为树节点执行                                │
│  ┌─────────────────┐  ┌─────────────────┐  ┌────────────────┐  │
│  │ IsGameStatus    │  │ IsEnergyEnough  │  │ PubNav2Goal    │  │
│  │ (读取黑板)       │  │ (读取黑板)       │  │ (发布话题)     │  │
│  └─────────────────┘  └─────────────────┘  └────────────────┘  │
└────────────────────┬───────────────────────────────────────────┘
                     │ (控制指令)
...
┌────────────────────────────────────────────────────────────────┐
│                控制话题 ← 行为树节点发布                         │
│         /goal_pose  /cmd_vel  /cmd_spin  /gimbal_cmd           │
└────────────────────┬───────────────────────────────────────────┘
                     │
                     ▼
┌────────────────────────────────────────────────────────────────┐
...
客户端 (Client)                     服务端 (Server)
     |                                    |
     |--- 1. sendGoal(target_tree) ------>|
     |                                    |--> onGoalReceived() 验证
     |                                    |--> 创建BehaviorTree
     |                                    |--> onTreeCreated() 初始化
     |                                    |
     |<--- 2. Feedback（循环） ------------|--- onLoopFeedback()
     |    feedbackCallback()              |    (每个tick周期)
     |                                    |
     |<--- 3. Result（完成） --------------|--- onTreeExecutionCompleted()
     |    resultCallback()                |
     |    timer_->reset()                 |
     |                                    |
     |--- 4. sendGoal（下一轮）-------——-->|

行为树资源分布：行为树 XML 文件放在包内 behavior_trees/*.xml，在参数 behavior_trees 中声明目录；行为树节点实现编译成共享库 pb2025_sentry_behavior/bt_plugins，在参数 plugins 中声明后由 TreeExecutionServer 动态加载并通过 BT_REGISTER_NODES / CreateRosNodePlugin 注册到工厂，XML 里用节点名引用它们，两者是松耦合的（名字匹配即可）。
...
rmul_2026 多点路径规划提示词：
理解pb2025_sentry_behavior项目代码，在rmul_2026.xml中写一套行为树

功能要求：比赛开始时（读取消息接口中的GameStatus.msg比赛状态数据，当前比赛状态由COUNT_DOWN = 3切换为RUNNING = 4的时候表示比赛开始），此时马上发送坐标点A(0,-4.76,0)并延时3秒等待机器人到达目标点A，后发送坐标点B(4.785,-4.76,0)并延时3秒等待机器人到达目标点B，后发送坐标点C(4.785,-3.01,0)并延时1.5秒等待机器人到达目标点C，机器人到达目标点C后，立刻发布PostureCmd.msg中的攻击姿态ATTACK = 1；在以上过程中，持续检测机器人当前血量(RobotStatus.msg中的current_hp)，如果当前血量小于150时，发布移动姿态MOVE = 3，依次发布坐标点C,B,A与坐标点D(0,0,0)返回起点，在C与B点之间延时1.5秒，其他点之间的发布延时3秒，到达起点D时延时4秒恢复血量；后继续按由A到B到C的路径与对应延时时间依次发布坐标点，到达点C后发布攻击姿态。

代码要求：
写两个子树和一个主树，主树包含比赛开始逻辑判断后循环tick两颗子树，第一条子树三由起点D到终点C，第二条子树由终点C返回起点D，第二条子树触发逻辑是血量小于150。